...
2026-09-14 · **MVP-01 · B-2 DONE** (14-09) — e-mail por tenant (RECEIVE desacoplado do Mailpit, provider por tenant) com QA independente **`QA_APPROVED_WITH_RESERVATIONS`** (0 blockers/high; F-1/F-2 MEDIUM + R-1..R-3 + follow-ups L/FU em `docs/factory/B2_FOLLOW_UPS.md`), **PO_ACCEPTED** (`HG-B2-2026-09_PO_ACCEPT`) e **PR #103 SQUASH MERGED — commit `0266322` (14/09)** com CI 4/4 verde; **HG-007 permanece `AWAITING_DECISION`**; **piloto NÃO GO**. Fix de infra pré-existente no merge: `apps/api/vitest.config.ts` exclui `dist/` (vitest 5.0.0 coletava artefato compilado quebrando `Testes` — falha já existia em `main`/`08628be`, fora do diff funcional B-2). Snapshot anterior: 2026-09-14 · **MVP-01 · B-2 PO_ACCEPTED** — e-mail por tenant com QA+PO aceitos e **merge PENDENTE** (rede GitHub indisponível na execução). Snapshot anterior: 2026-09-13 · **MVP-01 · B-1 MERGED** — fluxo `recebido → resolvido | excecao` (validação humana, Alternativa A / `HG-B1-2026-09`) implementado, revisão independente read-only **PASS COM RESSALVAS** (ressalvas tratadas), CI 4/4 verde, **PR #99 squash merge `04329db` (13/09)** — autorização de execução registrada (**HG-B1-2026-09_EXEC**); detalhes no relatório `docs/reports/B1_IMPLEMENTATION_REPORT_2026-09.md`. Snapshot anterior: 2026-09-12 · **RECONCILIAÇÃO OFICIAL** do estado real do GitHub (ver `docs/reports/FACTORY_V2_GITHUB_RECONCILIATION_2026-09.md`). Snapshots prévios: 2026-09-08 · P0.3-D (#57 ASVS) MERGED (PR #90, rebase L3, `a8c057a`, `Closes #57`) · Blueprint UX/UI Fase 2 MERGED (PR #92, squash L3, `731c009`) · P0.3-C (#56) e #73 merged via L2 · PRM-P0.3-A (#54, PR #80 `6313cab`) e PRM-P0.3-B (#55, PR #81 `69e0950`) DONE+MERGED · **M0 UX IMPLEMENTADO + VALIDADO (PR #96)** · Factory V2 operacional (Orchestrator + estados V2).

Mudanças desde o último snapshot (evidência objetiva em GitHub/git):

- **PR #103 (MVP-01 · B-2 — e-mail por tenant) `QA_APPROVED_WITH_RESERVATIONS` + `PO_ACCEPTED` + **MERGED (14-09, squash `0266322`)** — Core do motor **desacoplado do Mailpit** (RECEIVE genérico via `Recebedor`/`ProviderResolver`), resolução por tenant (gmail com credencial+token OAuth, mailpit, fallback dev/CI), **PollWorker único** (roubo cruzado de jobs corrigido), migração `0012_email_integration.sql` + `GET/PUT /configuracoes/integracao-email` (admin, RLS FORCE), envio por tenant (`sender_email`, fallback `MAIL_FROM`). QA independente read-only: suíte API **171+ passed/173** (26 files verdes), Runtime E2E **3/3**, build/typecheck ok, lint-diff 0 erros (9 erros só em `demo/` untracked — FU-1), rate-limit ok. `HG-B2-2026-09_PO_ACCEPT` registrado no `HUMAN_DECISIONS_LOG`; follow-ups em `docs/factory/B2_FOLLOW_UPS.md`. **CI 4/4 VERDE no merge (Lint, Lint+Typecheck+Build+Test, Selenium E2E, dívida-lint). Merge exigiu fix de infra pré-existente: vitest 5.0.0 (base `458b8ef`, bump #97) coletava artefato compilado `dist/app.controller.spec.js` e quebrava `Testes` (`Vitest cannot be imported in a CommonJS module`) — falha já presente em `main`/`08628be`, fora do diff funcional B-2; corrigido em `apps/api/vitest.config.ts` (`exclude: ['**/dist/**']`, commit `68224d5`, revalidado com vitest 5.0.0 local = 171 passed). HG-007 continua `AWAITING_DECISION`; piloto NÃO GO.**
...
| **HG-B2-2026-09** | **MVP-01 · B-2 — e-mail por tenant (RECEIVE desacoplado do Mailpit)** | ✅ QA_APPROVED_WITH_RESERVATIONS (14-09) + **PO_ACCEPTED (`HG-B2-2026-09_PO_ACCEPT`, 14-09)** + **MERGED (PR #103 squash `0266322`, 14/09, CI 4/4 verde)** · **HG-007 AWAITING_DECISION** |
...
| 14/09/2026 | merge PR squash (PO aprovou) | MVP-01 · B-2 (e-mail por tenant, HG-B2-2026-09) | [PR #103](https://github.com/rnsilveira22/servium/pull/103) `0266322` — QA_APPROVED_WITH_RESERVATIONS + PO_ACCEPTED · CI 4/4 · fix infra vitest pré-existente | ok |

> **L2 merges**: #76/#77/#78, #84, #81, #89, #91, #93, #95. **Merges L3 humanos (2026-09-08)**: PR #90 (ASVS #57, rebase `a8c057a`) e PR #92 (Blueprint Fase 2, squash `731c009`). **Merges humanos (09/09–10/09)**: PR #96 (`95c8160`) e PR #98 (`dfb75c0`). **Merge B-1 (13/09)**: PR #99 (`04329db`, PO aprovou). **Merge B-2 (14/09)**: PR #103 (`0266322`, PO aprovou). **PRs abertos (14/09)**: **#97 (dependabot/development) — fila NÃO zerada**; **sem PR para `feat/m1-frente-a-pleno`** (M1 UX implementada, aguarda decisão).
...
| **PR #103 (B-2 e-mail por tenant)** | Merge (PO aceitou) | **MERGED (14/09) — squash `0266322`, CI 4/4 verde**; B-2 `DONE`. **HG-007 permanece AWAITING_DECISION; piloto NÃO GO** |

## Próximos passos

0. **B-2 DONE (PR #103 merged, `0266322`, 14/09)** — squash merge + `main` reconciliada; **HG-007 permanece `AWAITING_DECISION`; piloto NÃO GO**. Próximo foco do MVP: fechamento do GO/NO-GO (HG-007, segurança do piloto, rollback/Stop, métricas mínimas, responsável humano, M1 UX, GO/NO-GO final) — **NÃO iniciar F-1/F-2 automaticamente** (ficam no backlog, CF `docs/factory/B2_FOLLOW_UPS.md`).
