...
| **HG-B2-2026-09** | **MVP-01 · B-2 — e-mail por tenant (RECEIVE desacoplado do Mailpit)** | ✅ QA_APPROVED_WITH_RESERVATIONS (14-09) + **PO_ACCEPTED (`HG-B2-2026-09_PO_ACCEPT`, 14-09)** · **merge do PR #103 PENDENTE (rede GitHub indisponível)** · **HG-007 AWAITING_DECISION** |
...
| **PR #103 (B-2 e-mail por tenant)** | Merge (PO aceitou) | `QA_APPROVED_WITH_RESERVATIONS` + `PO_ACCEPTED` (14/09, `HG-B2-2026-09_PO_ACCEPT`) + governança commitada no branch. **Merge/PUSH bloqueado por indisponibilidade de rede para GitHub** (github.com/api.github.com:443 inacessíveis); executar push → atualizar PR → CI → squash merge → reconciliar `main` quando a rede permitir; **HG-007 permanece AWAITING_DECISION** |

## Próximos passos

0. **Merge do PR #103 (B-2)**: com a rede GitHub disponível — `git push origin feat/mvp01-b2-email-provider` → atualizar PR #103 (QA `APPROVED_WITH_RESERVATIONS`, PO `ACCEPTED`, `MERGE: AUTHORIZED`) → aguardar CI verde (réplica local já verde) → squash merge → reconciliar `main` → declarar B-2 `DONE`. **HG-007 permanece `AWAITING_DECISION`; piloto NÃO GO.**
